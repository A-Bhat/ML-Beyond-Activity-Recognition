...
These variables give a cummulative measure of other variables whose measurements are recorded instantaneously. Hence these variables(the ones mentioned above) are highly related to the other variables in the dataset. So we eliminate all such variables containing cummulative readings.
...
confusionMatrix(predRF, testing$classe)$overall[1] #Accuracy of the model on the testing set
```

#### Gradient Boosting Machine(GBM):

```{r message=FALSE}
modBoost<-gbm(classe~., n.trees = 1000, data = training)

predBoost<-predict(modBoost, n.trees = 5000, type = "response", newdata = testing) 
predBoost<-apply(predBoost[,,1],1,which.max)
predBoost<-factor(predBoost,labels = c("A","B","C","D","E")) #Testing set predictions 

confusionMatrix(predBoost, testing$classe)$overall[1] #Accuracy of the model on the testing set
```
...
confusionMatrix(predLDA$class, testing$classe)$overall[1] #Accuracy of the model on the testing set

```


We now combine the testing set prediction of the base learners.

```{r}
predDF<-data.frame(predRF = predRF, predBoost = predBoost, predLDA = predLDA$class, 
                   classe = testing$classe)
```
